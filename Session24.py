...
class Customer:
    ...
    def input_details(self, old_customer=None):
        ...
        if old_customer is not None:
            ...
            # if we don't give any input,it will print us us previous name
            ...
            if len(temp) == 0:
                self.temperature = old_customer.temperature
            else:
...
def main():
    ...
    while True:
        ...
        if choice == 1:
            customer = Customer()
            sql = "insert into Customer values(null, '{name}', '{phone}', '{email}', {temperature}, '{intime}', '{outtime}')".format_map(
                vars(customer))
            ...
            sql = "update Customer set name='{name}', phone='{phone}', email='{}', temperature={temperature}, intime='{intime}', outtime='{outtime}' where id = {id}".format_map(
                vars(customer_to_update))
            my_db.execute_sql_write_operation(sql)
